...
pub fn send_server_handshake_packet() -> Vec<u8> {
  ...
  buf.push(0);//(conn_id as u8)>>8);
  buf.push(0);//(conn_id as u8)>>16);
  buf.push(0);//(conn_id as u8)>>24);
  // data = append(data, byte(c.connectionId), byte(c.connectionId>>8), byte(c.connectionId>>16), byte(c.connectionId>>24))

  // //auth-plugin-data-part-1

  buf.extend_from_slice(&[0x73, 0x64, 0x79, 0x53, 0x72, 0x63, 0x7c, 0x65, 0x00]);
  // let salt = [117, 41, 65, 18, 17, 39, 40, 50, 97, 103, 46, 122, 74, 5, 93, 115, 13, 75, 33, 91];
  // buf.extend_from_slice(&salt[0..7]);
  // buf.push('\0' as u8);
  ...
  buf.extend_from_slice(&[0x64, 0x45, 0x6e, 0x77, 0x2d, 0x78, 0x36, 0x42, 0x29, 0x45, 0x61, 0x39, 0x00]);
  // buf.push('\0' as u8);
